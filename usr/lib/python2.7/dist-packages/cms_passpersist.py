...
#
# This contains modifications made by Cumulus Networks, specifically:
#    - to handle termination as per the pass_persist protocol
#    - handle getNext more robustly for various OID snippets
#    - using ordereddict() to store the OIDs instead of sorting them
#      as the default sort put them in alphabetical order
#    - running the update on startup instead of waiting for the timer
#
...
__all__ = [ "encode", "start", "add_oid_entry", "add_int", "add_str",
            "add_cnt", "add_dispstr", "add_timestamp", "add_timeint" ]
...
class PassPersist:
	...
	def __init__(self, base_oid):
		...
		# The data structure is a dict that hold the sorted MIB tree like this :
                # (the MIB tree is sorted because it was inserted in the sorted order)
		# data = { 
	...
	def add_oid_entry(self, oid, type, value):
		...
		#self.data.update(oid,{'type': str(type), 'value': str(value)})
	...
	def main_passpersist(self):
            ...
            if 'PING' in line:
                ...
                if oid is None:
                    ...
                    print self.get(self.data_idx[0])
                else:
